...
if (isset($_GET['buscar'])) {
    ...
    $query = $conexion->prepare("SELECT * FROM feligreses 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = $conexion->prepare("INSERT INTO feligreses 
...
$query_iglesia = $conexion->prepare("SELECT * FROM iglesias WHERE id = ?");
$query_iglesia->execute([$id_iglesia]);
$iglesia = $query_iglesia->fetch(PDO::FETCH_ASSOC);

// Obtener feligreses de ESA iglesia
$query_feligreses = $conexion->prepare("SELECT * FROM feligreses WHERE id_iglesia = ?");
